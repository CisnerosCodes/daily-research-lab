"""MQAR joint LR scaling. Deterministic, CPU-only, writes results.json + chart.png.

Question (the named next step of 2026-08-07_mqar-qkv-lr-ratio): backbone x4 moved mean
escape 1100 -> 533 - causal in both directions but SUBLINEAR (exact inverse predicts
~275) - and at escape the gate had traveled only half its usual distance (||W|| ~11 vs
~21.6). Two candidate explanations the parent could not separate:

  (a) gate-lag: the gate is a serial dependency that still learns at 1x; when the
      backbone runs 4x the gate becomes the (partially) binding constraint, eating the
      other factor of ~2.
  (b) optimizer-side: Adam's approximate LR-time-reparametrization is broken by weight
      decay / gradient noise / curvature, so NO uniform speedup gives exact inverse
      scaling - the gate is irrelevant even at matched speeds (consistent with the
      2026-08-03 gate-LR flatness).

Decisive arm: scale gate AND backbone LR together by 4 (j4) on byte-identical paired
inits, alongside exact replications of the parent's blr1 (here j1) and blr4 (here b4).

Interpretation matrix (escape step at N=8, eval grid 100):
  j4 ~ 275-300 (~exact inverse of j1)   -> gate-lag confirmed: sublinearity at fixed
                                           gate LR was the gate lagging the backbone
  j4 ~ b4 (~533)                        -> optimizer-side: uniform x4 buys no more than
                                           backbone-only x4; gate LR irrelevant at speed
  j4 between                            -> partial decomposition; report the split
  j4 unstable/NaN/worse plateau         -> LR ceiling reached; read only stable arms
Secondary: within-arm constancy of qkv+out travel at escape (49.29/49.63/49.29 at 1x in
the parent) - does it hold inside j4, and where does j4's travel-at-escape land relative
to b4's elevated ~71.5?
...
def build_model(gated, d_model, n_layers, n_heads, mlp_exp, vocab, max_len, gate_bias):
    ...
    Byte-identical code path to 2026-07-28/29/08-01/08-03/08-07 (their 'none'/'dense' arms)."""
    import torch
...
def run_one(arm, N, seed, P):
    import torch
    d = P["d_model"]
    name, gmult, bmult, init_key = (arm["name"], arm["gate_lr_mult"],
                                    arm["backbone_lr_mult"], arm["init_key"])
    gated = init_key == "dense"
    # deterministic init seed; sum(ord) not hash() (2026-07-27 fix). CRUCIAL: the seed uses
    # init_key, not the arm name, so all gated arms are byte-identical per seed (and 'j1'/
    # 'b4'/'none' reproduce the 2026-08-07 'blr1'/'blr4'/'none' rows exactly).
    ...
    # per-parameter-group LR: backbone at bmult x base, gate at gmult x base (mirror of
    # 08-03 gate-side and 08-07 backbone-side; both dials live in the optimizer only).
    if gated:
...
def make_chart(runs, P, headline):
    ...
    cmap = {"j1": "#6a3d9a", "b4": "#1f78b4", "j4": "#e31a1c", "none": "#888888"}
    mstyle = {0: "-", 1: "--", 2: ":"}
    ...
    e1 = [r["escape_step"] for r in runs if r["arm"] == "j1" and r["escape_step"]]
    if e1:
        anchor = float(np.mean(e1))
        ax1.axhline(anchor / 4.0, color="#33a02c", lw=1.2, ls="-.", alpha=0.8,
                    label=f"exact 1/4 scaling ({anchor/4:.0f})")
    ax1.set_xticks(list(xpos.values()), fam)
    ax1.set_xlabel("arm (gate LR x, backbone LR x): j1=(1,1)  b4=(1,4)  j4=(4,4)")
    ax1.set_ylabel(f"escape step (first eval >= {P['escape_threshold']}, grid {P['eval_every']})")
    ax1.legend(frameon=False, fontsize=8)
    ax1.spines[["top", "right"]].set_visible(False)
    ax1.set_title("does joint x4 restore exact inverse scaling?", fontsize=10)
    ...
    # panel 3: attention-circuit travel at escape + gate ||W|| at escape, per arm
    for a in fam:
        rs = [r for r in runs if r["arm"] == a and r["num_pairs"] == Nm
              and r["travel_at_escape"] is not None]
        for r in rs:
            ax3.scatter(xpos[a] - 0.08, r["travel_at_escape"]["attn"], color=cmap.get(a),
                        s=45, alpha=0.85, marker="o")
            if r["travel_at_escape"]["gate_W"] is not None:
                ax3.scatter(xpos[a] + 0.08, r["travel_at_escape"]["gate_W"], color=cmap.get(a),
                            s=45, alpha=0.85, marker="s")
    ax3.scatter([], [], color="#555555", marker="o", label="qkv+out travel @ escape")
    ax3.scatter([], [], color="#555555", marker="s", label="gate ||W|| @ escape")
    ax3.set_xticks(list(xpos.values()), fam)
    ax3.set_xlabel("arm")
    ax3.set_ylabel("distance from init at escape eval")
    ax3.legend(frameon=False, fontsize=8)
    ax3.spines[["top", "right"]].set_visible(False)
    ax3.set_title("travel at escape: does within-arm constancy survive?", fontsize=10)
...
def main():
    ...
    if os.environ.get("MQAR_PILOT"):
        P.update(train_steps=100, eval_every=50,
                 arms=[{"name": "none", "gate_lr_mult": None, "backbone_lr_mult": None,
                        "init_key": "none", "seeds": [0]},
                       {"name": "j4", "gate_lr_mult": 4.0, "backbone_lr_mult": 4.0,
    ...
    for arm in P["arms"]:
        for s in arm["seeds"]:
            ...
            runs.append(r)
            print(f"[{time.time()-t0:7.1f}s] {r['arm']:5s} seed={s} acc={r['acc']:.3f} "
    ...
    import numpy as np
    Nm = P["num_pairs_main"]
    mean_acc, escapes, n_esc, attn_travel_esc, gate_travel_esc = {}, {}, {}, {}, {}
    for a in P["arms"]:
        name = a["name"]
