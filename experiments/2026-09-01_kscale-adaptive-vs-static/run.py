"""Is the running-scale win adaptive normalization, or just a better key scale?

Follow-up to the 2026-09-01 head sweep, where k_emascale (alpha frozen at 1: keys divided by a
per-head EMA of their RMS, no per-token norm) matched FKN at hd=4. Arms separate (i) a static
per-head scale fixed from the first batch (init-time rescale), (ii) the EMA at three momenta
(adaptation speed), (iii) the same trick on the query side, and (iv) a learnable per-head
scalar (the free-parameter control, cf. 08-31 kgain_only).
...
class Block(nn.Module):
    ...
    def __init__(self, d, n_head, d_ff, norm_q, norm_k, dyn_q, dyn_k,
        ...
        super().__init__()
        assert d % n_head == 0
        assert dyn_q in (None, "undetached", "alpha_fixed1", "alpha_fixed1_frozen")
        assert dyn_k in (None, "undetached", "alpha_fixed1", "alpha_fixed1_frozen")
    ...
    def _update_ema(self, ema, ready, r_det, frozen=False):
        if self.training:
            with torch.no_grad():
                ...
                if float(ready) == 0.0:
                    ...
                    ready.fill_(1.0)
                elif not frozen:   # "alpha_fixed1_frozen": keep the first-batch scale forever
    ...
    def _qkv(self, x, return_stats=False):
        ...
        q = q.view(*shape4)
        if self.dyn_q:
            self._update_ema(self.q_rms_ema, self.q_ema_ready, rq_det, frozen=(self.dyn_q == "alpha_fixed1_frozen"))
        ...
        if self.norm_k:
            k4 = self._rms(k4)
            if self.dyn_k:
                self._update_ema(self.k_rms_ema, self.k_ema_ready, rk_det, frozen=(self.dyn_k == "alpha_fixed1_frozen"))
...
class GPT(nn.Module):
    def __init__(self, vocab, d, n_layer, n_head, d_ff, block_size, arm_flags,
        ...
        super().__init__()
        norm_q, norm_k, dyn_q, dyn_k = arm_flags[:4]
        if len(arm_flags) > 4 and arm_flags[4] is not None:
            ema_momentum = float(arm_flags[4])
...
def make_chart(m, p, headline, path):
    ...
    labels = {"baseline": "baseline (no norm)", "qknorm": "QK-norm", "qnorm_only": "q-norm only",
              ...
              "qknorm_dynq": "QK-norm + dynq (08-11)", "k_emascale": "k / EMA-scale (m=0.99)",
              "k_static_init_scale": "k / first-batch scale (static)", "k_emascale_m09": "k / EMA-scale (m=0.9)",
              "k_emascale_m0999": "k / EMA-scale (m=0.999)", "q_emascale": "q / EMA-scale (m=0.99)"}
    colors = {"baseline": "#444444", "qknorm": "#8e44ad", "qnorm_only": "#3498db",
              "knorm_only": "#27ae60", "knorm_dynk": "#c0392b", "qknorm_dynq": "#e67e22",
              "k_emascale": "#7f8c8d", "k_static_init_scale": "#2c3e50", "k_emascale_m09": "#95a5a6",
              "k_emascale_m0999": "#34495e", "q_emascale": "#f1c40f"}
    styles = {"baseline": "--", "qknorm": "-", "qnorm_only": ":", "knorm_only": "-",
              "knorm_dynk": "-", "qknorm_dynq": "-.", "k_emascale": ":", "k_static_init_scale": "--",
              "k_emascale_m09": ":", "k_emascale_m0999": ":", "q_emascale": "-."}
    by = m["by_arm_hd"]
    x = np.arange(len(hds))
