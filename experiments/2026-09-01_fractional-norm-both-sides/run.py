"""Is the fractional channel side-specific? Query-only fractional norm and both-sides fractional norm across the head split.

Follow-up to the 2026-09-01 head sweep (FKN = key-only fractional norm beats baseline at every width; the
08-11 composite QK-norm + query channel is best at hd 16-32 but fails at hd 4 and 128). Arms: fqn = query-only
fractional norm (the mirror of FKN, the backlog's qnorm-dynq-overshoot-check); fqkn = fractional norm on BOTH
sides with independent learned exponents.
...
def make_chart(m, p, headline, path):
    ...
    ax.set_ylabel("val bits per character (600 steps)")
...
# ------------------------------------------------------------------------------ main
IMPORT_FROM = HERE.parent / "2026-09-01_knorm-dynk-head-sweep" / "results.json"
IMPORT_ARMS = ("baseline", "qknorm", "knorm_only", "knorm_dynk", "qknorm_dynq", "k_emascale")


def merge(cfg, p):
    """Merge tonight's parts, then import the comparison arms from the same-night head sweep.

    The two harnesses are byte-identical (same Block, same recipe, same seeds and batch stream),
    and the head sweep reproduced 68/68 archived cells to <= 0.0005 bpc on this machine, so the
    imported cells are the values these arms would produce if rerun here. Imported runs carry
    "imported_from" so the provenance is explicit in results.json."""
    runs, seen = [], set()
    for part in sorted(glob.glob(str(HERE / "results_part_*.json"))):
        for r in json.load(open(part))["runs"]:
            ...
            if key not in seen:
                ...
                runs.append(r)
    n_own = len(runs)
    if IMPORT_FROM.exists():
        for r in json.load(open(IMPORT_FROM))["runs"]:
            key = (r["arm"], r["head_dim"], r["seed"])
            if r["arm"] in IMPORT_ARMS and r["head_dim"] in p["head_dims"] and key not in seen:
                r = dict(r); r["imported_from"] = IMPORT_FROM.parent.name
                seen.add(key); runs.append(r)
    print(f"merged {n_own} runs from parts + {len(runs) - n_own} imported comparison cells")
    return runs
...
def main():
    ...
    if args.merge:
        p["arm_order"] = [a for a in IMPORT_ARMS] + [a for a in p["arm_order"] if a not in IMPORT_ARMS]
