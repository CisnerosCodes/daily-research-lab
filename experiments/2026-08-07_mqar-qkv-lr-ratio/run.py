"""MQAR backbone LR ratio. Deterministic, CPU-only, writes results.json + chart.png.

Question (the named next step of 2026-08-03_mqar-gate-lr-ratio): gate-LR dose-response
showed escape pins to ~step 1000 for every gate multiplier m >= 1 - the gate is a floor,
not a throttle - and INFERRED that the binding constraint (the clock) is the backbone
(qkv/out/MLP/emb), not the gate. This experiment is the causal test of that inference:
fix the gate LR at 1x and multiply ONLY the backbone LR by {0.25, 1, 4}.

Design (inherits the 2026-08-01/08-03 pairing upgrade): the LR multiplier lives entirely
in the optimizer, so every gated arm shares the same init key ("dense") and the same data
stream. Within a seed all blr* runs are byte-identical at step 0 and see identical batches;
any divergence is caused by the backbone LR alone. blr1 is an exact replication of
2026-08-03's "glr1" (= 2026-08-01's "dense" arm).

Extra readout: backbone travel (sum of ||p - p0|| over qkv/out attention params, and over
the whole backbone) is logged at EVERY eval alongside gate travel, so we can ask whether
escape happens at roughly constant backbone travel across multipliers (backbone travel is
the clock) or at a constant step (the clock is elsewhere / the story needs revising).

Interpretation matrix:
  escape step ~inverse in backbone mult      -> backbone learning is the clock; 2026-08-03's
                                                inference becomes causal
  escape flat in backbone mult               -> neither subsystem's LR alone sets the clock;
                                                revises the 2026-08-03 story
  blr4 collapses/NaN or degrades the plateau -> LR ceiling confound; read escape only where
                                                training is stable
  backbone travel at escape ~constant        -> travel-as-clock holds on the backbone side
                                                (it failed on the gate side in 2026-08-03)
...
def build_model(gated, d_model, n_layers, n_heads, mlp_exp, vocab, max_len, gate_bias):
    ...
    Byte-identical code path to 2026-07-28/29/08-01/08-03 (their 'none' and 'dense' arms)."""
    import torch
...
def run_one(arm, N, seed, P):
    import torch
    d = P["d_model"]
    name, mult, init_key = arm["name"], arm["backbone_lr_mult"], arm["init_key"]
    gated = init_key == "dense"
    # deterministic init seed; sum(ord) not hash() (2026-07-27 fix). CRUCIAL: the seed uses
    # init_key, not the arm name, so all gated arms are byte-identical per seed (and 'blr1'/
    # 'none' reproduce the 2026-08-03 'glr1'/'none' rows exactly).
    ...
    # per-parameter-group LR: BACKBONE params at mult x base, gate at base (mirror of 08-03).
    if gated:
        ...
        opt = torch.optim.AdamW(
            [{"params": rest_ps, "lr": P["lr"] * mult},
             {"params": gate_ps, "lr": P["lr"]}],
    ...
    for step in range(1, P["train_steps"] + 1):
        ...
        if step % P["eval_every"] == 0:
            ...
            travel_traj.append([step, gw, gb, ba, bf])
    ...
    if escape is not None and travel_traj:
        travel_at_escape = next(
            ({"gate_W": gw, "gate_b": gb, "attn": ba, "backbone": bf}
             for s, gw, gb, ba, bf in travel_traj if s >= escape), None)
    ba_end, bf_end = backbone_travel(model, snap)
    return {"arm": name, "backbone_lr_mult": mult, "num_pairs": N, "seed": seed,
...
def make_chart(runs, P, headline):
    ...
    mult_of = {a["name"]: a["backbone_lr_mult"] for a in P["arms"] if a["init_key"] == "dense"}
    cmap = {"blr025": "#e31a1c", "blr1": "#6a3d9a", "blr4": "#1f78b4", "none": "#888888"}
    mstyle = {0: "-", 1: "--", 2: ":"}
    ...
    # panel 1: escape step vs backbone LR mult, one line per seed (paired inits)
    for s in seeds_all:
        xs, ys, censored = [], [], []
        for a in fam:
    ...
    # reference: exact inverse scaling anchored at the blr1 seed-mean escape
    e1 = [r["escape_step"] for r in runs if r["arm"] == "blr1" and r["escape_step"]]
    if e1:
        anchor = float(np.mean(e1))
        mm = np.array(sorted(mult_of.values()), dtype=float)
        ax1.plot(mm, np.minimum(anchor / mm, cens), color="#33a02c", lw=1.2, ls="-.",
                 alpha=0.8, label="exact 1/mult scaling")
    ax1.set_xscale("log", base=2)
    ax1.set_xticks([0.25, 1, 4], ["0.25x", "1x", "4x"])
    ax1.axhline(cens, color="#e31a1c", lw=0.6, ls=":", alpha=0.5)
    ax1.set_xlabel("backbone LR multiplier (gate LR fixed 1e-3)")
    ax1.set_ylabel(f"escape step (first eval >= {P['escape_threshold']}, grid {P['eval_every']})")
    ax1.legend(frameon=False, fontsize=8)
    ax1.spines[["top", "right"]].set_visible(False)
    ax1.set_title("escape step vs backbone LR (paired inits per seed)", fontsize=10)
    ...
    # panel 3: attention-circuit (qkv+out) travel at escape, per arm (the clock readout)
    for a in fam:
        rs = [r for r in runs if r["arm"] == a and r["num_pairs"] == Nm
              and r["travel_at_escape"] is not None]
        for r in rs:
            ax3.scatter(mult_of[a], r["travel_at_escape"]["attn"], color=cmap.get(a),
                        s=45, alpha=0.85)
    ax3.set_xscale("log", base=2)
    ax3.set_xticks([0.25, 1, 4], ["0.25x", "1x", "4x"])
    ax3.set_xlabel("backbone LR multiplier")
    ax3.set_ylabel("qkv+out travel  sum ||p - p0||  at escape eval")
    ax3.spines[["top", "right"]].set_visible(False)
    ax3.set_title("is escape at constant attention-circuit travel?", fontsize=10)
...
def main():
    ...
    if os.environ.get("MQAR_PILOT"):
        P.update(train_steps=100, eval_every=50,
                 arms=[{"name": "none", "backbone_lr_mult": None, "init_key": "none", "seeds": [0]},
                       {"name": "blr4", "backbone_lr_mult": 4.0, "init_key": "dense", "seeds": [0]}])
    t0 = time.time()
    runs = []
    ...
    for arm in P["arms"]:
        for s in arm["seeds"]:
            ...
            print(f"[{time.time()-t0:7.1f}s] {r['arm']:7s} seed={s} acc={r['acc']:.3f} "
                  ...
                  f"travel@esc={r['travel_at_escape']} bb_travel@end={r['backbone_travel_end']} "
                  f"({r['secs']}s)", flush=True)

    # aggregates
    import numpy as np
    Nm = P["num_pairs_main"]
    mean_acc, escapes, n_esc, attn_travel_esc = {}, {}, {}, {}
    for a in P["arms"]:
        name = a["name"]
